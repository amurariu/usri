...
# This version of aldex.effect is used in '0_figDAunpermuted.R' to calculate the 
# median CLR values for 21,813 genes (across 128 Monte-Carlo instances) in 224
# patient-derived breast tissue samples from the corresponding aldex.clr object, 
# without also calculating within & between group median CLR differences and 
# effect sizes for all genes. Using the complete aldex.effect() function on this 
# data causes RStudio to throw a fatal error (likely due to memory exhaustion) 
# on a 2023 MacBook Pro (M2 chip, 16 GB RAM). This truncated function was used 
# as a workaround. As a result, arguments 'CI', 'glm.conds' and 'paired.test'
# are  now functionally irrelevant and DO NOT change the output of the function.

# The same function was used previously in Dos Santos et al. (2024) to obtain
# CLR abundance values for ~31,000 genes in 341 vaginal metatranscriptomes. More
# info on the latter study, along with the identical code can be found at: 
# https://github.com/scottdossantos/dossantos2024study
