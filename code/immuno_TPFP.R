...
model <- which(abs(input$thin.data[[i]]$coefmat) > coeff)  
...
FP.r[[i]] <- intersect(which(input$r.data[[i]][,padj] < 0.05), null.model.r)
...
#for randomized + permuted data (p)
PPV.pr <- length(TP.p)/sum(length(TP.p),length(FP.pr))
PPV.pp <- length(TP.p)/sum(length(TP.p),length(FP.pp))
FDR.pr <- length(FP.pr)/sum(length(TP.p),length(FP.pr))
FDR.pp <- length(FP.pp)/sum(length(TP.p),length(FP.pp))
SEN.p <- length(TP.p)/(length(TP.p) + length(FN.p)) #same for both?
SPE.pr <- length(TN.pr)/(length(TN.pr) + length(FP.pr))
SPE.pp <- length(TN.pp)/(length(TN.pp) + length(FP.pp))
