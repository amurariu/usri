...
%DIF DEL /Users/ggloor/Downloads/Manuscript.tex        Tue Aug 25 10:06:41 2026
%DIF ADD ../PLoS_revised_3rd/diffAbund_msRevised.tex   Thu Aug 27 10:11:03 2026
...
\\
\textbf{2} \DIFaddbegin \DIFadd{Program in Bioinformatics and Genomics, Pennsylvania State University, University Park, Pennsylvania, USA
}\\
\textbf{\DIFadd{3}} \DIFaddend College of Information Science and Technology, Pennsylvania State University, State College, \DIFdelbegin \DIFdel{PA}\DIFdelend \DIFaddbegin \DIFadd{Pennsylvania}\DIFaddend , USA
\\
\textbf{\DIFdelbegin \DIFdel{3}\DIFdelend \DIFaddbegin \DIFadd{4}\DIFaddend } Department of Statistics, Pennsylvania State University, State College, \DIFdelbegin \DIFdel{PA}\DIFdelend \DIFaddbegin \DIFadd{Pennsylvania}\DIFaddend , USA
\\
\textbf{\DIFdelbegin \DIFdel{4}\DIFdelend \DIFaddbegin \DIFadd{5}\DIFaddend } Department of Medicine, Pennsylvania State University, Hershey, \DIFdelbegin \DIFdel{PA}\DIFdelend \DIFaddbegin \DIFadd{Pennsylvania}\DIFaddend , USA
\\
\textbf{\DIFdelbegin \DIFdel{5}\DIFdelend \DIFaddbegin \DIFadd{6}\DIFaddend } Institute for Computational and Data Science, Pennsylvania State University, State College, \DIFdelbegin \DIFdel{PA}\DIFdelend \DIFaddbegin \DIFadd{Pennsylvania}\DIFaddend , USA
\\
\bigskip
...
All code needed to reproduce these analyses are available on GitHub at: \url{https://github.com/amurariu/usri}. This repository also contains input count data and metadata for all datasets, as well as code needed to reproduce the figures in this study. \DIFaddbegin \DIFadd{We further provide accession numbers or direct links to the original data as provided by the respective study authors: PD1 immunotherapy and TCGA datasets (Li }\textit{\DIFadd{et al.}}\DIFadd{\mbox{%DIFAUXCMD
\cite{li_exaggerated_2022}}\hskip0pt%DIFAUXCMD
) = }\url{https://zenodo.org/records/8320659} \DIFadd{(Zenodo);  yeast transcriptome (Gierlinksi }\textit{\DIFadd{et al.}}\DIFadd{\mbox{%DIFAUXCMD
\cite{gierlinski_statistical_2015}}\hskip0pt%DIFAUXCMD
) = }\url{https://www.ebi.ac.uk/ena/browser/view/PRJEB5348} \DIFadd{(ENA); vaginal metatranscriptome (Dos Santos }\textit{\DIFadd{et al.}}\DIFadd{\mbox{%DIFAUXCMD
\cite{dos_santos_vaginal_2024}}\hskip0pt%DIFAUXCMD
) = }\url{https://github.com/scottdossantos/dossantos2024study} \DIFadd{and }\url{https://dbgap.ncbi.nlm.nih.gov/beta/search/?OBJ=study&TERM=phs001523.v2.p1} \DIFadd{(dbGaP); single-cell RNA-seq dataset (Kang }\textit{\DIFadd{et al.}}\DIFadd{\mbox{%DIFAUXCMD
\cite{kang_multiplexed_2018}}\hskip0pt%DIFAUXCMD
) = }\url{https://www.ncbi.nlm.nih.gov/sra/?term=PRJNA381100} \DIFadd{(NCBI SRA); 16S rRNA amplicon dataset (Bian }\textit{\DIFadd{et al.}}\DIFadd{\mbox{%DIFAUXCMD
\cite{bian_gut_2017}}\hskip0pt%DIFAUXCMD
) = }\url{https://www.ncbi.nlm.nih.gov/sra/?term=PRJNA385551} \DIFadd{(NCBI SRA). }\DIFaddend All figures were produced using ggplot2 (v3.5.2) \cite{wickham_ggplot_2016} and edited in Inkscape (v1.3.1). For transparency, details of figure edits made in Inkscape are available on GitHub, in addition to the final SVG files.
...
To explain both the decrease in significantly different features and the superior FDR control that characterise ALDEx2 and ALDEx3, we constructed volcano plots \cite{Cui:2003aa} (difference between groups vs. -log\textsubscript{10} \textit{P}-value) and effect plots \cite{gloor:effect} (difference within groups vs. difference between groups) from a randomly chosen analysis iteration of the PD1 immunotherapy dataset. When comparing ALDEx3 volcano plots of significantly different features defined by a dual \textit{P}-value/fold-change cutoff (Fig \DIFdelbegin \DIFdel{. }\DIFdelend \ref{fig:volcanoEffect}A) to those identified by employing a scale model (Fig \DIFdelbegin \DIFdel{. }\DIFdelend \ref{fig:volcanoEffect}B), one observes that scale models achieve the same result without unnecessarily inflating the FDR \cite{ebrahimpoor_inflated_2021}. Further, there exist distinct, curved boundaries between features that were only significantly different between groups in the scale-na\"ive analysis, and features which remain differential when some degree of scale uncertainty is modelled (similar boundaries can also be seen in  \DIFdelbegin \DIFdel{Supplementary }\DIFdelend \nameref{fig:supFigEffectUnperm}). By explicitly modelling uncertainty in the scale, both tools eliminated potentially spurious false-positive results by a dynamic cutoff that: i) required a large L2FC if near the FDR cutoff value chosen (i.e. features have a large L2FC but marginal statistical significance) or ii) allowed a smaller L2FC value if the adjusted \textit{P}-value was smaller (i.e. features are more statistically significant, but not necessarily biologically pertinent). Note that if a feature is reported as differentially expressed at a larger gamma value, it will always be reported as differential when using a smaller value of gamma. The same phenomenon is illustrated in effect plots (Fig \DIFdelbegin \DIFdel{. }\DIFdelend \ref{fig:volcanoEffect}C and \ref{fig:volcanoEffect}D): features that exhibit a larger dispersion within a group require a larger difference between groups to remain significantly different at larger \textgamma~values. Essentially, there is a boundary of significance represented by the ratio between the ALDEx3 estimate (i.e. L2FC) and its standard error which is shifted outwards as the degree of scale uncertainty increases (Fig \DIFdelbegin \DIFdel{. }\DIFdelend \ref{fig:volcanoEffect}D), with a larger ratio required to achieve statistical significance as gamma increases. Scale models implemented in ALDEx2 show the same behaviour (\DIFdelbegin \DIFdel{Supplementary Fig . }\DIFdelend \DIFaddbegin \DIFadd{Fig }\DIFaddend \nameref{fig:supFigVolcanoEffectA2}).
...
\paragraph*{\DIFdelbegin \DIFdel{Fig. }\DIFdelend S1 \DIFaddbegin \DIFadd{Fig}\DIFaddend }
...
\paragraph*{\DIFdelbegin \DIFdel{Fig. }\DIFdelend S2 \DIFaddbegin \DIFadd{Fig}\DIFaddend }
...
\paragraph*{\DIFdelbegin \DIFdel{Fig. }\DIFdelend S3 \DIFaddbegin \DIFadd{Fig}\DIFaddend }
...
\paragraph*{\DIFdelbegin \DIFdel{Fig. }\DIFdelend S4 \DIFaddbegin \DIFadd{Fig}\DIFaddend }
...
\paragraph*{\DIFdelbegin \DIFdel{Table }\DIFdelend S1 \DIFaddbegin \DIFadd{Table}\DIFaddend }
\label{tab:supTabDensity}
\textbf{Average minimum log differences required for a significant result with ALDEx2 and ALDEx3 for all datasets and \textgamma~values.} For all datasets, the mean minimum log difference required for ALDEx2 and ALDEx3 to recognise a feature as differentially expressed (i.e. BH-adjusted \textit{P}-value $<$0.05) was calculated for \textgamma~values ranging from 0 to 0.5. Data represent a mean of all 100 iterations of permuted/thinned HTS count data (TGCA, The Cancer Genome Atlas).

\paragraph*{\DIFdelbegin \DIFdel{Fig. }\DIFdelend S5 \DIFaddbegin \DIFadd{Fig}\DIFaddend }
\label{fig:supFigDispAbund}
\textbf{Features from single-cell and metatranscriptome datasets have a larger dispersion compared to bulk RNA-seq data.} The within-group log dispersion of each feature was plotted against the log abundance from a single, randomly selected iteration of the ALDEx2 permutation analyses for all HTS datasets.

\paragraph*{\DIFdelbegin \DIFdel{Fig. }\DIFdelend S6 \DIFaddbegin \DIFadd{Fig}\DIFaddend }
...
\paragraph*{\DIFdelbegin \DIFdel{Table }\DIFdelend S2 \DIFaddbegin \DIFadd{Table}\DIFaddend }
...
\paragraph*{\DIFdelbegin \DIFdel{Fig. }\DIFdelend S7 \DIFaddbegin \DIFadd{Fig}\DIFaddend }
...
\paragraph*{\DIFdelbegin \DIFdel{File }\DIFdelend S1 \DIFaddbegin \DIFadd{File}\DIFaddend }
...
\paragraph*{\DIFdelbegin \DIFdel{Fig. }\DIFdelend S8 \DIFaddbegin \DIFadd{Fig}\DIFaddend }
