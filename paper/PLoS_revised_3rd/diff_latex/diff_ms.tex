...
%DIF DEL old_ms.tex   Thu Jul 30 13:51:04 2026
%DIF ADD new_ms.tex   Tue Aug 11 09:19:51 2026
...
) = }\url{https://zenodo.org/records/8320659}\DIFadd{;  yeast transcriptome (Gierlinksi }\textit{\DIFadd{et al.}}\DIFadd{\mbox{%DIFAUXCMD
\cite{gierlinski_statistical_2015}}\hskip0pt%DIFAUXCMD
) = PRJEB5348 (ENA); vaginal metatranscriptome (Dos Santos }\textit{\DIFadd{et al.}}\DIFadd{\mbox{%DIFAUXCMD
\cite{dos_santos_vaginal_2024}}\hskip0pt%DIFAUXCMD
) = }\url{https://github.com/scottdossantos/dossantos2024study} \DIFadd{and phs001523.v2.p1 (dbGaP); single-cell RNA-seq dataset (Kang }\textit{\DIFadd{et al.}}\DIFadd{\mbox{%DIFAUXCMD
\cite{kang_multiplexed_2018}}\hskip0pt%DIFAUXCMD
) = PRJNA381100 (NCBI SRA); 16S rRNA amplicon dataset (Bian }\textit{\DIFadd{et al.}}\DIFadd{\mbox{%DIFAUXCMD
\cite{bian_gut_2017}}\hskip0pt%DIFAUXCMD
) = PRJNA385551 (NCBI SRA). }\DIFaddend All figures were produced using ggplot2 (v3.5.2) \cite{wickham_ggplot_2016} and edited in Inkscape (v1.3.1). For transparency, details of figure edits made in Inkscape are available on GitHub, in addition to the final SVG files.
...
\textbf{Average minimum log differences required for a significant result with ALDEx2 and ALDEx3 for all datasets and \textgamma~values.} For all datasets, the mean minimum log difference required for ALDEx2 and ALDEx3 to recognise a feature as differentially expressed (i.e. BH-adjusted \textit{P}-value $<$0.05) was calculated for \textgamma~values ranging from 0 to 0.5\DIFdelbegin \DIFdel{was calculated for \textgamma~values ranging from 0 to 0.5}\DIFdelend . Data represent a mean of \DIFdelbegin \DIFdel{either: }\DIFdelend all 100 iterations of permuted/thinned \DIFdelbegin \DIFdel{RNA-seq data ; or all 21 pairwise combinations of non-permuted, non-thinned 16S rRNA amplicon sequencing data }\DIFdelend \DIFaddbegin \DIFadd{HTS count data }\DIFaddend (TGCA, The Cancer Genome Atlas).

\paragraph*{Fig. S5}
\label{fig:supFigDispAbund}
\textbf{Features from single-cell and metatranscriptome datasets have a larger dispersion compared to bulk RNA-seq data.} The within-group log dispersion of each feature was plotted against the log abundance from \DIFdelbegin \DIFdel{either: 1) }\DIFdelend a single, randomly selected iteration of the ALDEx2 permutation analyses for \DIFdelbegin \DIFdel{the single-cell, metatranscriptome, PD1 immunotherapy, and yeast transcriptome datasets; or 2) one of the unique pairwise age group comparisons from the Tianyi 16S rRNA amplicon dataset (selected due to its regression coefficient and intercept being closest to the dataset averages of both parameters)}\DIFdelend \DIFaddbegin \DIFadd{all HTS datasets}\DIFaddend .
