...
%DIF DEL old_ms.tex   Thu May 28 09:46:33 2026
%DIF ADD new_ms.tex   Thu Jul 30 13:51:04 2026
...
%
\section*{Abstract}
Differential expression/abundance analyses are commonplace in studies employing high-throughput sequencing \DIFaddbegin \DIFadd{(HTS)}\DIFaddend ; however different tools often fail to return comparable results when applied to the same dataset. Most tools employ normalisations to attempt to correct for technical variation in the count data. Previously, we demonstrated that these normalisations are often inappropriate due to incorrect assumptions regarding the overall scale (i.e. size) of the biological system in question. In this study, we used a combination of binomial thinning and permutation of sample groupings to produce 100 \DIFdelbegin \DIFdel{iterations of 10 }\DIFdelend \DIFaddbegin \DIFadd{analysis iterations of 11 }\DIFaddend RNA-seq \DIFaddbegin \DIFadd{and other HTS }\DIFaddend datasets in which $\sim$5 \% of all features are expected to be significantly different between groups. This enabled calculation of the false discovery rate (FDR) and sensitivity across \DIFdelbegin \DIFdel{100 analysis }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend iterations. Our simulations showed that scale misspecification results in poor control of the FDR by several commonly used tools and that\DIFdelbegin \DIFdel{this could be ameliorated by using }\DIFdelend \DIFaddbegin \DIFadd{, counterintuitively, FDRs increased as the modelled difference between groups increased. Implementing }\DIFaddend a scale model in ALDEx2 or ALDEx3 \DIFdelbegin \DIFdel{to account for uncertainty around the system scale. We highlighted the }\DIFdelend \DIFaddbegin \DIFadd{ameliorated unacceptably high FDRs; however, there was an }\DIFaddend inherent trade-off between satisfactory \DIFdelbegin \DIFdel{control of the FDR and high sensitivity, showing that }\DIFdelend \DIFaddbegin \DIFadd{FDR control and high sensitivity- }\DIFaddend no tool offered both. We established that increasing scale uncertainty also increased the minimum difference between groups required for a feature to be reported as differentially expressed. This phenomenon was not only universal, but also remarkably consistent between disparate types of sequencing data. Critically, we leveraged \DIFdelbegin \DIFdel{‘}\DIFdelend \DIFaddbegin \DIFadd{a `}\DIFaddend real-world’, non-permuted \DIFdelbegin \DIFdel{analyses of bulk }\DIFdelend \DIFaddbegin \DIFadd{analysis of an }\DIFaddend RNA-seq \DIFdelbegin \DIFdel{and 16S rRNA amplicon datasets }\DIFdelend \DIFaddbegin \DIFadd{dataset }\DIFaddend to demonstrate that the latter effect is not a result of our thinning/permutation approach. Overall, our work highlights the \DIFdelbegin \DIFdel{(potentially unwitting ) }\DIFdelend \DIFaddbegin \DIFadd{potentially unwitting }\DIFaddend choice between sensitivity and FDR control that all researchers are making when analysing sequencing data and provides guidance on choosing an appropriate amount of scale uncertainty for the analysis of any type of \DIFdelbegin \DIFdel{high-throughput sequencing }\DIFdelend \DIFaddbegin \DIFadd{HTS }\DIFaddend data.
...
Despite attempts to define best practices in differential abundance or expression analysis of high-throughput sequencing data, there is no standardised approach, and a plethora of tools are available to use. Each tool applies different assumptions and methods, which can lead to drastically different results when multiple tools are applied to the same dataset. Our work \DIFdelbegin \DIFdel{echoes several recent studies describing how }\DIFdelend \DIFaddbegin \DIFadd{reproduces multiple recent studies showing that }\DIFaddend several popular tools exaggerate differences in gene expression or abundance between groups. We build on prior work by showing that these excessively high false discovery rates can be mitigated by using a new approach that models some degree of uncertainty around the total size (or scale) of the biological system under study. We find that scale models offer excellent false-discovery rate control, though \DIFdelbegin \DIFdel{are less sensitive, and that no }\DIFdelend \DIFaddbegin \DIFadd{at a reduced sensitivity. No }\DIFaddend tool offers both high sensitivity and a low rate of false positives, highlighting a trade-off that researchers must be cognisant of when analysing \DIFdelbegin \DIFdel{sequencing }\DIFdelend \DIFaddbegin \DIFadd{HTS }\DIFaddend data. Accordingly, we extend these observations with guidance on selecting an appropriate degree of scale uncertainty so that analysts can make an informed choice about balancing the ability to detect differences in their data with the confidence that any given difference is real.
...
Previous work highlighting the advantages of the SRI framework has mainly focused on amplicon sequencing and shotgun metagenomic datasets \cite{konnaris_uncertainty_2025}, or has been limited to a single transcriptomic dataset \cite{gloor_explicit_2025}. Here, we extend prior studies by testing the performance of scale models from ALDEx2, and its faster and more memory-efficient successor, ALDEx3, on \DIFdelbegin \DIFdel{10 different RNA-seq }\DIFdelend \DIFaddbegin \DIFadd{11 different HTS }\DIFaddend datasets, encompassing approaches such as bulk transcriptome (from both lab-derived and clinical samples), single-cell \DIFdelbegin \DIFdel{and metatranscriptome}\DIFdelend \DIFaddbegin \DIFadd{transcriptome, metatranscriptome, and 16S rRNA amplicon }\DIFaddend sequencing. We compare these two implementations of SRI to widely-used, normalisation-based tools in terms of sensitivity and FDR, and highlight an important trade-off between the two that researchers should consider when analysing HTS data. Further, we quantify the relationship between the degree of scale uncertainty modelled by ALDEx2 and ALDEx3 and the minimum fold-change between conditions required for a statistically significant result- a critical factor to consider when conducting differential expression analysis with scale models. This analysis provides strong guidance on how to build appropriate scale models that offer better FDR controls for HTS data analysis.


\section*{Materials and methods}

\subsection*{Datasets used in analysis}
We obtained read count data and grouping metadata from publicly available RNA-seq datasets described by the following studies or sources: Li \textit{et al.} \cite{li_exaggerated_2022} (PD1 immunotherapy dataset), Gierlinski \textit{et al.} \cite{gierlinski_statistical_2015} (wild-type vs. \textit{Snf2}-knockout yeast transcriptome dataset), \DIFdelbegin \DIFdel{Skinnider }\DIFdelend \DIFaddbegin \DIFadd{Kang }\DIFaddend \textit{et al.} \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{skinnider_evaluating_2019} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{kang_multiplexed_2018} }\hskip0pt%DIFAUXCMD
}\DIFaddend (single-cell RNA-seq dataset comparing \DIFdelbegin \DIFdel{cytotoxic T cells vs. memory T cells}\DIFdelend \DIFaddbegin \DIFadd{peripheral blood mononuclear cells with and without interferon beta treatment}\DIFaddend ), Dos Santos \textit{et al.} \cite{dos_santos_vaginal_2024} (vaginal metatranscriptome dataset comparing healthy vs. bacterial vaginosis patients, aggregated by KEGG orthology terms), and Bian \textit{et al.} \cite{bian_gut_2017} (16S rRNA amplicon sequencing data from stool samples obtained from Chinese individuals of different age groups). A further \DIFdelbegin \DIFdel{seven }\DIFdelend \DIFaddbegin \DIFadd{six }\DIFaddend bulk RNA-seq datasets originating from the Cancer Genome Atlas project \cite{weinstein_cancer_2013} were obtained from the supplementary data provided by Li \textit{et al}. via Zenodo \cite{li_supplement_2022} (available at: \url{https://zenodo.org/records/8320659}), corresponding to the following tumour types: breast invasive carcinoma (BRCA) \cite{tcga_brca_2012,tcga_brca_2015}, kidney renal cell carcinoma (KIRC) \cite{tcga_kirc_2013}, liver hepatocellular carcinoma (LIHC) \cite{tcga_lihc_2017}, lung adenocarcinoma (LUAD) \cite{tcga_luad_2016}, prostate adenocarcinoma (PRAD) \cite{tcga_prad_2015}, and thyroid carcinoma (THCA) \cite{tcga_thca_2014}.

All Cancer Genome Atlas datasets, the PD1 immunotherapy dataset, and the yeast transcriptome dataset were filtered using the \DIFaddbegin \DIFadd{`}\DIFaddend \texttt{filterByExpr()}\DIFaddbegin \DIFadd{' }\DIFaddend function from the edgeR package \cite{robinson_edger_2010}. The single-cell RNA-seq dataset \DIFdelbegin \DIFdel{was filtered such that only 1000 randomly selected cells in the upper quartile of read counts were retained in each group,along with features with read count totals of at least 300 reads across all samples}\DIFdelend \DIFaddbegin \DIFadd{first underwent pseudobulking using the Seurat package \mbox{%DIFAUXCMD
\cite{stuart_comprehensive_2019} }\hskip0pt%DIFAUXCMD
and aggregated counts were filtered to retain the 8,500 most median-expressed features in the dataset using the `}\texttt{\DIFadd{select\_counts()}}\DIFadd{' function built into the }\texttt{\DIFadd{seqgendiff}} \DIFadd{package}\DIFaddend . The vaginal metatranscriptome dataset was filtered using the CoDaSeq package (v0.99.7) \cite{gloor_codaseq} to retain features present in 30 \% of samples, at a minimum proportion of 0.005 \% of a given sample (as per the original study) \cite{dos_santos_vaginal_2024}. The 16S rRNA amplicon dataset \DIFdelbegin \DIFdel{used the same set of operational taxonomic units as in the original analysis \mbox{%DIFAUXCMD
\cite{bian_gut_2017}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{was filtered to retain the 400 most median-expressed features in the dataset using the `}\texttt{\DIFadd{select\_counts()}}\DIFadd{' function built into the }\texttt{\DIFadd{seqgendiff}} \DIFadd{package}\DIFaddend .

\subsection*{Binomial thinning of count data and group membership permutation}
For the analysis of all RNA-seq datasets, we \DIFdelbegin \DIFdel{applied binomial thinning via }\DIFdelend \DIFaddbegin \DIFadd{employed }\DIFaddend the seqgendiff R package (v1.2.4) \cite{gerard_data-based_2020} to achieve the following: i) permute the grouping variable such that group membership of all samples is randomised; and ii) inject an artificial signal into read count data \DIFaddbegin \DIFadd{using binomial thinning }\DIFaddend such that 5 \% of all features in a given dataset are modelled to be differentially abundant between groups. \DIFdelbegin \DIFdel{The result of the former is that }\DIFdelend \DIFaddbegin \DIFadd{Consequently, }\DIFaddend any analysis with the permuted grouping variable is meaningless as almost all positive findings are, by definition, false-positives (FPs; excluding features affected by binomial thinning). \DIFdelbegin \DIFdel{The result of the latter is that }\DIFdelend \DIFaddbegin \DIFadd{Likewise, }\DIFaddend any features whose counts are altered by \DIFdelbegin \DIFdel{binomial thinning }\DIFdelend \DIFaddbegin \DIFadd{the process }\DIFaddend \emph{could} be detected as being significantly different between groups and are considered simulated `true' positives (TPs). The magnitude of the difference between groups (i.e. the amount of `signal' added) is drawn from a normal distribution with a \DIFdelbegin \DIFdel{standard deviation of 2 and a mean of zero}\DIFdelend \DIFaddbegin \DIFadd{user-specified mean and standard deviation}\DIFaddend ; because of this, not all features which have been altered by thinning are actually detectable by the differential expression tools (i.e. false negatives, FNs). Features whose counts have not been altered by binomial thinning and are not identified as significantly different between groups are `true' negatives (TNs). \DIFaddbegin \DIFadd{For these analyses, thinning was performed using a normal distribution with a mean of 0 and a standard deviation of either 2 (RNA-seq datasets) or 4 (16S rRNA dataset; due to the higher variance).
}\DIFaddend 
...
\DIFdelbegin \DIFdel{Programmatically}\DIFdelend \DIFaddbegin \DIFadd{In ALDEx2}\DIFaddend , $\gamma$ is controlled by the `\texttt{gamma}' parameter in the call to `\texttt{aldex.clr()}', which controls the standard deviation of the Gaussian distribution referenced above. By default, $\mu_n$ is set to 0, but may be specified by making a custom matrix of system scale values with the `\texttt{aldex.makeScaleMatrix()}' command. The `\texttt{gamma}' parameter can be used to incorporate only uncertainty in the estimation of normalisation, or can be tuned to include prior information about absolute abundance in each sample along with the error of those observations (see Nixon et al. \cite{nixon_incorporating_2025}, for a fulsome description of the \DIFaddbegin \DIFadd{`}\DIFaddend \texttt{gamma}\DIFaddbegin \DIFadd{' }\DIFaddend parameter). For \DIFdelbegin \DIFdel{both ALDEx2 and }\DIFdelend ALDEx3, \DIFdelbegin \DIFdel{simple scale models can be constructed by passing a single numeric value }\DIFdelend \DIFaddbegin \DIFadd{scale models are generated by passing scale model functions }\DIFaddend to the `\DIFaddbegin \texttt{\DIFadd{aldex()}}\DIFadd{' command, along with the `}\DIFaddend \texttt{gamma}' \DIFdelbegin \DIFdel{argument. These scale values are used to adjust the log-ratio transformed count data}\DIFdelend \DIFaddbegin \DIFadd{parameter; in this work, we use the `}\texttt{\DIFadd{clr.sm}}\DIFadd{' model to more closely match the default behaviour of ALDEx2 (see the ALDEx3 documentation for further information)}\DIFaddend . In the present work, gamma values from essentially zero (0.001) to 0.5 in increments of 0.1 were used for both tools to model biological system scale. 

\subsection*{Data availability and visualisation}
All code needed to reproduce these analyses are available on GitHub at: \url{https://github.com/amurariu/usri}. This repository also contains input count data and metadata for all datasets, as well as code needed to \DIFdelbegin \DIFdel{replicate }\DIFdelend \DIFaddbegin \DIFadd{reproduce }\DIFaddend the figures in this study. All figures were produced using ggplot2 (v3.5.2) \cite{wickham_ggplot_2016} and edited in Inkscape (v1.3.1). For transparency, details of figure edits made in Inkscape are available on GitHub, in addition to the \DIFdelbegin \DIFdel{original }\DIFdelend \DIFaddbegin \DIFadd{final }\DIFaddend SVG files.
...
We next calculated the true-positive rate (TPR), or sensitivity, for all tool and dataset combinations. Unlike what was observed for the FDR calculations, DESeq2, edgeR and limma exhibited higher sensitivity in every dataset tested than did either version of ALDEx\DIFdelbegin \DIFdel{(Fig. \ref{fig:TPR}). Conversely, sensitivity was lower for both ALDEx2 and ALDEx3 across every dataset, }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend even in scale-na\"ive analyses  \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{(Fig. \ref{fig:TPR}). }\DIFaddend This was expected because the ALDEx model is a consensus model \cite{fernandes:2014} and generally has a somewhat lower sensitivity \cite{hawinkel2017, Nearing:2022aa}. As was observed for FDR control, the addition of scale uncertainty had an evident effect on TPRs: increasing gamma values reduced sensitivity, with larger values (\textgamma~$\geq$0.4) resulting in less than 50 \% of true positive features being detected by ALDEx2 in some datasets. ALDEx3 performed better than its predecessor at every degree of scale uncertainty tested, remaining competitive with DESeq2, edgeR and limma when small amounts of scale were added (\textgamma~$\leq$0.2)\DIFaddbegin \DIFadd{, especially in bulk transcriptome datasets}\DIFaddend . Notably, ALDEx2 and ALDEx3 had poor sensitivity with the single-cell dataset and to a lesser extent, the vaginal metatranscriptome dataset (though the latter may be due to the fact that a simple scale model is insufficient for the vaginal metatranscriptome; see discussion). TPRs for \DIFdelbegin \DIFdel{these datasets }\DIFdelend \DIFaddbegin \DIFadd{the single-cell and metatranscriptome analyses }\DIFaddend were markedly lower than \DIFdelbegin \DIFdel{the remaining bulk transcriptome data (Supplementary }%DIFDELCMD < \nameref{fig:supFigFDR_TPR_all}%%%
\DIFdelend \DIFaddbegin \DIFadd{other datasets (Supplementary }\nameref{fig:supFigTPR_all}\DIFaddend ), failing to achieve $>$\DIFdelbegin \DIFdel{40 }\DIFdelend \DIFaddbegin \DIFadd{30 }\DIFaddend and $>$50 \%, respectively, in many cases. Notably, the effect of the modelled L2FC on sensitivity was again obvious. The TPR tended to plateau for all tools when the modelled L2FC was $\geq$0.5, although at high \textgamma~values $\ge$0.4, this was not the case. 
...
To explain both the decrease in significantly different features and the superior FDR control that characterise ALDEx2 and ALDEx3, we constructed volcano plots \cite{Cui:2003aa} (difference between groups vs. -log\textsubscript{10} \textit{P}-value) and effect plots \cite{gloor:effect} (difference within groups vs. difference between groups) from a randomly chosen analysis iteration of the PD1 immunotherapy dataset. \DIFdelbegin \DIFdel{For both tools, the volcano plots show }\DIFdelend \DIFaddbegin \DIFadd{When comparing ALDEx3 volcano plots of significantly different features defined by a dual }\textit{\DIFadd{P}}\DIFadd{-value/fold-change cutoff (Fig. \ref{fig:volcanoEffect}A) to those identified by employing a scale model (Fig. \ref{fig:volcanoEffect}B), one observes that scale models achieve the same result without unnecessarily inflating the FDR \mbox{%DIFAUXCMD
\cite{ebrahimpoor_inflated_2021}}\hskip0pt%DIFAUXCMD
. Further, there exist }\DIFaddend distinct, curved boundaries between \DIFdelbegin \DIFdel{genes that were }\DIFdelend \DIFaddbegin \DIFadd{features that were only }\DIFaddend significantly different between groups \DIFdelbegin \DIFdel{at increasing amounts }\DIFdelend \DIFaddbegin \DIFadd{in the scale-na\"ive analysis, and features which remain differential when some degree }\DIFaddend of scale uncertainty \DIFdelbegin \DIFdel{(Fig. \ref{fig:volcano}A and \ref{fig:volcano}B). The same approximate boundaries for each \textgamma~value in additional datasets }\DIFdelend \DIFaddbegin \DIFadd{is modelled (similar boundaries }\DIFaddend can also be seen in Supplementary \DIFdelbegin %DIFDELCMD < \nameref{fig:supFigEffect}%%%
\DIFdelend \DIFaddbegin \nameref{fig:supFigEffectUnperm}\DIFadd{)}\DIFaddend . By explicitly modelling uncertainty in the scale, both tools eliminated potentially spurious false-positive results by a dynamic cutoff that: i) required a large L2FC if near the FDR cutoff value chosen (i.e. features have a large L2FC but marginal statistical significance) or ii) allowed a smaller L2FC value if the adjusted \textit{P}-value was smaller (i.e. features are more statistically significant, but not necessarily biologically pertinent). Note that if a feature is reported as differentially expressed at a larger gamma value, it will always be reported as differential when using a smaller value of gamma. The same phenomenon is illustrated in effect plots (Fig. \DIFdelbegin \DIFdel{\ref{fig:volcano}C and \ref{fig:volcano}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:volcanoEffect}C and \ref{fig:volcanoEffect}}\DIFaddend D): features that exhibit a larger dispersion within a group require a larger difference between groups to remain significantly different at larger \textgamma~values. \DIFdelbegin \DIFdel{This effect is less pronounced with }\DIFdelend \DIFaddbegin \DIFadd{Essentially, there is a boundary of significance represented by the ratio between the }\DIFaddend ALDEx3 \DIFdelbegin \DIFdel{, explaining its increased sensitivity even at higher degrees }\DIFdelend \DIFaddbegin \DIFadd{estimate (i.e. L2FC) and its standard error which is shifted outwards as the degree }\DIFaddend of scale uncertainty \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{increases (Fig. \ref{fig:volcanoEffect}D), with a larger ratio required to achieve statistical significance as gamma increases. Scale models implemented in ALDEx2 show the same behaviour (Supplementary Fig. }\nameref{fig:supFigVolcanoEffectA2}\DIFadd{).
}\DIFaddend 

\begin{figure}[H]
    \centering
    \caption{\DIFdelbeginFL \textbf{\DIFdelFL{Accounting for scale uncertainty reduces the total number of features identified as differentially expressed.}} %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{ALDEx3 scale models provide better FDR control than a dual }\textit{\DIFaddFL{P}}\DIFaddFL{-value/fold-change cutoff.}} \DIFaddendFL A randomly selected \DIFaddbeginFL \DIFaddFL{analysis }\DIFaddendFL iteration \DIFdelbeginFL \DIFdelFL{from }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{of }\DIFaddendFL the PD1 immunotherapy dataset was used to create volcano plots (\textbf{A, B}) and effect plots (\textbf{C, D}) for \DIFdelbeginFL \DIFdelFL{the corresponding ALDEx2 and }\DIFdelendFL ALDEx3 analyses. Each data point represents a feature from the scale-na\"ive analysis (\textgamma~= 0.001). \DIFaddbeginFL \DIFaddFL{Differentially expressed features are defined either by: a BH-adjusted }\textit{\DIFaddFL{P}}\DIFaddFL{-value threshold of $<$0.05 }\textit{\DIFaddFL{and}} \DIFaddFL{a fold-change cutoff of $\geq$1 (}\textbf{\DIFaddFL{A, B}}\DIFaddFL{); or a BH-adjusted }\textit{\DIFaddFL{P}}\DIFaddFL{-value threshold of $<$0.05 when \textgamma = 0.5 (}\textbf{\DIFaddFL{C, D}}\DIFaddFL{). }\DIFaddendFL Grey points indicate non-significant features\DIFdelbeginFL \DIFdelFL{; coloured }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{. Coloured }\DIFaddendFL points indicate features which are significantly different between groups after \DIFdelbeginFL \DIFdelFL{a given amount }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{applying different definitions }\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{scale uncertainty has been added to the model}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{differential expression}\DIFaddendFL . \DIFdelbeginFL \DIFdelFL{Grey }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Vertical }\DIFaddendFL dashed lines \DIFaddbeginFL \DIFaddFL{in }\textbf{\DIFaddFL{A-C}} \DIFaddendFL indicate \DIFdelbeginFL \DIFdelFL{either: }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{a log$_2$ fold-change of 1; horizontal dashed lines indicate }\DIFaddendFL BH-adjusted \textit{P}-value $<$0.05\DIFdelbeginFL \DIFdelFL{(}\textbf{\DIFdelFL{A, B}}%DIFAUXCMD
\DIFdelFL{) or equivalent difference }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{. Grey dotted and dashed lines in }\textbf{\DIFaddFL{D}} \DIFaddFL{indicate approximate isarithms }\DIFaddendFL between \DIFaddbeginFL \DIFaddFL{non-significant }\DIFaddendFL and \DIFdelbeginFL \DIFdelFL{within groups (}\textbf{\DIFdelFL{C, D}}%DIFAUXCMD
\DIFdelFL{)}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{significant features when \textgamma{} = 0.001 and 0.5, respectively}\DIFaddendFL .}
    \DIFdelbeginFL %DIFDELCMD < \label{fig:volcano}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{fig:volcanoEffect}
...
\begin{table}[H]
    ...
    \begin{tabular}{lrrrr}
         ...
         Tianyi 16S rRNA & \textbf{\DIFdelbeginFL \DIFdelFL{2.61}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{2.00}\DIFaddendFL } (\DIFdelbeginFL \DIFdelFL{0.85}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1.57}\DIFaddendFL ) & \textbf{\DIFdelbeginFL \DIFdelFL{0.64}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1.73}\DIFaddendFL } (\DIFdelbeginFL \DIFdelFL{0.14}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.58}\DIFaddendFL ) & \textbf{\DIFdelbeginFL \DIFdelFL{1.66}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1.37}\DIFaddendFL } (\DIFdelbeginFL \DIFdelFL{0.37}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1.44}\DIFaddendFL ) & \textbf{\DIFdelbeginFL \DIFdelFL{0.72}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1.84}\DIFaddendFL } (\DIFdelbeginFL \DIFdelFL{0.11}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.60}\DIFaddendFL ) \\
         Single-cell & \textbf{\DIFdelbeginFL \DIFdelFL{2.53}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{2.35}\DIFaddendFL } (\DIFdelbeginFL \DIFdelFL{1.11}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.65}\DIFaddendFL ) & \textbf{\DIFdelbeginFL \DIFdelFL{0.46}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.79}\DIFaddendFL } (\DIFdelbeginFL \DIFdelFL{0.16}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.19}\DIFaddendFL ) & \textbf{\DIFdelbeginFL \DIFdelFL{1.95}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1.61}\DIFaddendFL } (\DIFdelbeginFL \DIFdelFL{0.97}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.75}\DIFaddendFL ) & \textbf{\DIFdelbeginFL \DIFdelFL{0.49}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.88}\DIFaddendFL } (\DIFdelbeginFL \DIFdelFL{0.19}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.24}\DIFaddendFL ) \\
...
\paragraph*{Fig. S1}
\DIFdelbegin %DIFDELCMD < \label{fig:supFigFDR_TPR_all}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{fig:supFigFDR_all}
\DIFaddend \textbf{False-discovery \DIFdelbegin \DIFdel{and true-positive }\DIFdelend rates for all datasets tested.} False-discovery rates (FDR\DIFdelbegin \DIFdel{; }\textbf{\DIFdel{A}}%DIFAUXCMD
\DIFdel{) and true positive rates (TPR; }\textbf{\DIFdel{B}}%DIFAUXCMD
\DIFdelend ) for all tool and dataset combinations were calculated separately for each 0.1 increment in the model difference between groups (i.e. the simulated or `true' log\textsubscript{2} fold-change between groups, as determined by binomial thinning). Each data point represents the FDR \DIFdelbegin \DIFdel{or TPR }\DIFdelend when considering dataset features with a simulated log-difference at or above the given x-axis threshold. Features were considered differentially abundant at a BH-adjusted \textit{P}-value of $<$0.05.

\paragraph*{Fig. S2}
\DIFdelbegin %DIFDELCMD < \label{fig:supFigPPV}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{fig:supFigPPV_all}
\DIFaddend \textbf{Positive predictive values for all datasets tested.} The positive predictive value (PPV) for all tool and dataset combinations was calculated separately for each 0.1 increment in the model difference between groups (i.e. the simulated or `true' log\textsubscript{2} fold-change between groups, as determined by binomial thinning). Each data point represents the PPV when considering dataset features with a simulated log-difference at or above the given x-axis threshold. Features were considered differentially abundant at a BH-adjusted \textit{P}-value of $<$0.05.

\DIFaddbegin \paragraph*{\DIFadd{Fig. S3}}
\label{fig:supFigTPR_all}
\textbf{\DIFadd{True-positive rates for all datasets tested.}} \DIFadd{True positive rates (TPR) for all tool and dataset combinations were calculated separately for each 0.1 increment in the model difference between groups (i.e. the simulated or `true' log\textsubscript{2} fold-change between groups, as determined by binomial thinning). Each data point represents the TPR when considering dataset features with a simulated log-difference at or above the given x-axis threshold. Features were considered differentially abundant at a BH-adjusted }\textit{\DIFadd{P}}\DIFadd{-value of $<$0.05.
}

\paragraph*{\DIFadd{Fig. S4}}
\label{fig:supFigVolcanoEffectA2}
\textbf{\DIFadd{ALDEx2 scale models provide better FDR control than a dual }\textit{\DIFadd{P}}\DIFadd{-value/fold-change cutoff.}} \DIFadd{A randomly selected analysis iteration of the PD1 immunotherapy dataset was used to create volcano plots (}\textbf{\DIFadd{A, B}}\DIFadd{) and effect plots (}\textbf{\DIFadd{C, D}}\DIFadd{) for ALDEx2 analyses. Each data point represents a feature from the scale-na\"ive analysis (\textgamma~= 0.001). Differentially expressed features are defined either by: a BH-adjusted }\textit{\DIFadd{P}}\DIFadd{-value threshold of $<$0.05 }\textit{\DIFadd{and}} \DIFadd{a fold-change cutoff of $\geq$1 (}\textbf{\DIFadd{A, B}}\DIFadd{); or a BH-adjusted }\textit{\DIFadd{P}}\DIFadd{-value threshold of $<$0.05 when \textgamma = 0.5 (}\textbf{\DIFadd{C, D}}\DIFadd{). Grey points indicate non-significant features. Coloured points indicate features which are significantly different between groups after applying different definitions of differential expression. Vertical dashed lines in }\textbf{\DIFadd{A-C}} \DIFadd{indicate a log$_2$ fold-change of 1; horizontal dashed lines indicate BH-adjusted }\textit{\DIFadd{P}}\DIFadd{-value $<$0.05. Grey dotted and dashed lines in }\textbf{\DIFadd{D}} \DIFadd{indicate approximate isarithms between non-significant and significant features when \textgamma{} = 0.001 and 0.5, respectively.
}

\DIFaddend \paragraph*{Table S1}
...
\paragraph*{Fig. \DIFdelbegin \DIFdel{S3}\DIFdelend \DIFaddbegin \DIFadd{S5}\DIFaddend }
...
\paragraph*{Fig. \DIFdelbegin \DIFdel{S4}\DIFdelend \DIFaddbegin \DIFadd{S6}\DIFaddend }
...
\paragraph*{Fig. \DIFdelbegin \DIFdel{S6}\DIFdelend \DIFaddbegin \DIFadd{S8}\DIFaddend }
