...
%DIF ADD new_ms.tex   Tue Jul 28 13:12:20 2026
...
%
\section*{Abstract}
Differential expression/abundance analyses are commonplace in studies employing high-throughput sequencing; however different tools often fail to return comparable results when applied to the same dataset. Most tools employ normalisations to attempt to correct for technical variation in the count data. Previously, we demonstrated that these normalisations are often inappropriate due to incorrect assumptions regarding the overall scale (i.e. size) of the biological system in question. In this study, we used a combination of binomial thinning and permutation of sample groupings to produce 100 iterations of 10 RNA-seq datasets in which $\sim$5 \% of all features are expected to be significantly different between groups. This enabled calculation of the false discovery rate (FDR) and sensitivity across 100 analysis iterations. Our simulations showed that scale misspecification results in poor control of the FDR by several commonly used tools and that this could be ameliorated by using a scale model in ALDEx2 or ALDEx3 to account for uncertainty around the system scale. We highlighted the inherent trade-off between satisfactory control of the FDR and high sensitivity, showing that no tool offered both. We established that increasing scale uncertainty also increased the minimum difference between groups required for a feature to be reported as differentially expressed. This phenomenon was not only universal, but also remarkably consistent between disparate types of sequencing data. Critically, we leveraged ‘real-world’, non-permuted analyses of bulk RNA-seq and 16S rRNA amplicon datasets to demonstrate that the latter effect is not a result of our thinning/permutation approach. Overall, our work highlights the (potentially unwitting) choice between sensitivity and FDR control that all researchers are making when analysing sequencing data and provides guidance on choosing an appropriate amount of scale uncertainty for the analysis of any type of high-throughput sequencing data.
...
Programmatically, $\gamma$ is controlled by the `\texttt{gamma}' parameter in the call to `\texttt{aldex.clr()}', which controls the standard deviation of the Gaussian distribution referenced above. By default, $\mu_n$ is set to 0, but may be specified by making a custom matrix of system scale values with the `\texttt{aldex.makeScaleMatrix()}' command. The `\texttt{gamma}' parameter can be used to incorporate only uncertainty in the estimation of normalisation, or can be tuned to include prior information about absolute abundance in each sample along with the error of those observations (see Nixon et al. \cite{nixon_incorporating_2025}, for a fulsome description of the \DIFaddbegin \DIFadd{`}\DIFaddend \texttt{gamma}\DIFaddbegin \DIFadd{' }\DIFaddend parameter). For both ALDEx2 and ALDEx3, simple scale models can be constructed by passing a single numeric value to the `\texttt{gamma}' argument. These scale values are used to adjust the log-ratio transformed count data. In the present work, gamma values from essentially zero (0.001) to 0.5 in increments of 0.1 were used for both tools to model biological system scale.

\subsection*{Data availability and visualisation}
All code needed to reproduce these analyses are available on GitHub at: \url{https://github.com/amurariu/usri}. This repository also contains input count data and metadata for all datasets, as well as code needed to replicate the figures in this study. All figures were produced using ggplot2 (v3.5.2) \cite{wickham_ggplot_2016} and edited in Inkscape (v1.3.1). For transparency, details of figure edits made in Inkscape are available on GitHub, in addition to the \DIFdelbegin \DIFdel{original }\DIFdelend \DIFaddbegin \DIFadd{final }\DIFaddend SVG files.
...
%DIFDELCMD < %%%
\DIFdel{It must also be noted that we do not advocate for the use of }\DIFdelend ALDEx2 \DIFdelbegin \DIFdel{or ALDEx3 for analysis of single-cell RNA-seq data. Rather, we included a single-cell dataset in this work to demonstrate the universal effect of scale uncertainty, even when dealing with data that have very different distributional characteristics compared to the other RNA-seq datasets }%DIFDELCMD < \nameref{fig:supFigDispAbund}%%%
\DIFdel{. ALDEx2 }\DIFdelend and ALDEx3 \DIFdelbegin \DIFdel{have }\DIFdelend \DIFaddbegin \DIFadd{had }\DIFaddend the lowest sensitivity overall, but also \DIFaddbegin \DIFadd{exhibited }\DIFaddend markedly lower FDR rates \DIFdelbegin \DIFdel{for all datasets . Note }\DIFdelend \DIFaddbegin \DIFadd{in all datasets tested. We note }\DIFaddend that high sensitivity is not itself useful if the majority of features are returned as significant; this situation is analogous to not being able to determine which runner won a race \DIFdelbegin \DIFdel{based on ribbons }\DIFdelend \DIFaddbegin \DIFadd{if medals are }\DIFaddend given out for participation alone. Notably, ALDEx3 provides the same functionality as its predecessor in terms of acknowledging biological scale during normalisation; however, it offers superior sensitivity at the same degree of scale uncertainty while also controlling for false discoveries at any given L2FC and \textgamma~value. \DIFaddbegin \DIFadd{Limma performed well in some but not all types of data, having an acceptable FDR and a high sensitivity in the majority of datasets. While the application of tools like DESeq2 and edgeR- designed for bulk RNA-seq- to single-cell data (rather than specialised tools for single-cell data) remains somewhat contentious \mbox{%DIFAUXCMD
\cite{murphy_balanced_2022}}\hskip0pt%DIFAUXCMD
, the application of these tools to count data at the cell level reflects a common (yet inappropriate) use case \mbox{%DIFAUXCMD
